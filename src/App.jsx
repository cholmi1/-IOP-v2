...
function PatientDetail({ p, role, onBack, devices, setDevices, patients, sent, onSend, onUpdatePatient, toast }) {
  ...
  const [homeFrom, setHomeFrom] = useState((p.period || "").split(" ~ ")[0] || RANGE_FROM_DEFAULT);
  const [homeTo, setHomeTo] = useState((p.period || "").split(" ~ ")[1] || RANGE_TO_DEFAULT);
  const perm = CAN[role];
  const pts = period === "custom" ? trendDataRange(from, to) : trendData(period);
  const gmeta = GRAPH_TYPES.find((g) => g.id === gtype);
    ...
    <div style={{ padding: "16px 20px" }}>
      ...
      {modal === "cert" && (
        <ConfirmModal title={p.certified ? "환자 인증 해제" : "환자 인증"} icon={ShieldCheck} tone={p.certified ? "warn" : "primary"}
          lines={p.certified
            ? [`${p.name}님의 인증을 해제합니다.`, "해제 후에는 측정 데이터가 의료진 웹으로 수신되지 않습니다.", "환자 앱에는 기록이 계속 저장됩니다."]
            : [`${p.name}님을 인증합니다.`, "인증 후 환자 앱의 측정 기록이 이 기관으로 전달됩니다.", "환자 본인 확인을 마친 뒤 진행하세요."]}
      ...
      {tab === "profile" && (
        <div className="grid grid-cols-2" style={{ gap: 12 }}>
          ...
          <div className="col-span-2 flex items-center gap-2" style={{ marginTop: 2 }}>
            <button onClick={() => setModal("pw")} className="cursor-pointer flex items-center gap-1.5" style={{ border: `1.5px solid ${C.line}`, background: "#fff", color: C.sub, borderRadius: 10, padding: "9px 14px", fontSize: 12.5, fontWeight: 700, fontFamily: FONT }}><KeyRound size={13} /> 비밀번호 재설정 메일 발송</button>
        ...
        <div className="flex flex-col gap-3">
          <Card style={{ padding: 15 }}>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setModal("cert")} className="cursor-pointer" style={{ border: "none", background: p.certified ? C.mintDeep : C.primary, color: p.certified ? C.primary : "#fff", borderRadius: 9, padding: "8px 14px", fontSize: 12, fontWeight: 800, fontFamily: FONT }}>{p.certified ? "인증 해제" : "인증하기"}</button>
            </div>
          </Card>
          <Card style={{ padding: 15 }}>
            <div style={{ fontSize: 13.5, fontWeight: 800, color: C.ink, marginBottom: 10 }}>홈 사용 기간</div>
            <div className="flex items-center gap-2.5">
              <Field label="시작일"><input type="date" value={homeFrom} max={homeTo} onChange={(e) => setHomeFrom(e.target.value)} style={inpSm} /></Field>
              <Field label="종료일"><input type="date" value={homeTo} min={homeFrom} onChange={(e) => setHomeTo(e.target.value)} style={inpSm} /></Field>
              <div style={{ paddingTop: 20 }}>
                <button onClick={() => { onUpdatePatient && onUpdatePatient(p.id, { period: `${homeFrom} ~ ${homeTo}` });
                  toast && toast(`홈 사용 기간을 ${homeFrom} ~ ${homeTo}로 지정했습니다.`); }}
                  disabled={`${homeFrom} ~ ${homeTo}` === p.period} className="cursor-pointer"
                  style={{ border: "none", background: `${homeFrom} ~ ${homeTo}` === p.period ? C.mintDeep : C.primary, color: "#fff", borderRadius: 9, padding: "9px 16px", fontSize: 12.5, fontWeight: 800, fontFamily: FONT }}>지정</button>
              </div>
            </div>
